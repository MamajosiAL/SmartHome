...
public class RoomDeleter  implements Callback<String> {
    ...
    public RoomDeleter(RoomActivity context) {
    ...
    private void startRequest(int roomId) {
        RoomsApiInterface roomApi = getClient().create(RoomsApiInterface.class);
        Call<String> call = roomApi.deleteRoomById(roomId, AuthorizationManager.getInstance(context).getSessionId());
        call.enqueue(this);
    }
    ...
    public void onResponse(Call<String> call, Response<String> response) {
        if (response != null && response.body() != null) {
            if (response.isSuccessful()) {
                ...
                intent.putExtra("roomId", context.getIntent().getIntExtra("roomId",0));
                intent.putExtra("houseId", context.getIntent().getIntExtra("houseId",0));
    ...
    public void delete() {
        ...
        houseId = context.getIntent().getIntExtra("houseId",0);
        houseName = context.getIntent().getStringExtra("houseName");
        if(roomId>0){
            startRequest(roomId);
        }else {
